...
class PreSetup(object):
    ...
    def _input_mysql(self):
        while True:
            mysql = input('是否安装新的MySQL服务器? (y/n) [y]: ')
            if mysql != 'n':
                self._setup_mysql()
            else:
                db_host = input('请输入数据库服务器IP [127.0.0.1]: ').strip()
                db_port = input('请输入数据库服务器端口 [3306]: ').strip()
                db_user = input('请输入数据库服务器用户 [jumpserver]: ').strip()
                db_pass = input('请输入数据库服务器密码: ').strip()
                db = input('请输入使用的数据库 [jumpserver]: ').strip()
    ...
    def _input_smtp(self):
        while True:
            self.mail_host = input('请输入SMTP地址: ').strip()
            mail_port = input('请输入SMTP端口 [25]: ').strip()
            self.mail_addr = input('请输入账户: ').strip()
            self.mail_pass = input('请输入密码: ').strip()

            if mail_port: self.mail_port = int(mail_port)

            if self._test_mail():
                color_print('\n\t请登陆邮箱查收邮件, 然后确认是否继续安装\n', 'green')
                smtp = input('是否继续? (y/n) [y]: ')
                if smtp == 'n':
                    continue
                else:
                    break
